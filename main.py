    ...
    def pool_thumb(self,i):
        ...
        if 'image' in magic.from_file(f, mime=True):  # extract all images
            ...
            try:
                ...
                im_data.thumbnail(self.size,Image.ANTIALIAS)
                __ = self.arg[1].rstrip("/") + "/" + i[:-1*len(im_data.format)-1] + ".thumbnail"
...
def main():
    ...
    parser = argparse.ArgumentParser(description=" \
             Multithreaded quick image processing from terminal",epilog="Author: tushar.rishav@gmail.com")
    group = parser.add_mutually_exclusive_group()
    group.add_argument("-i", "--image", nargs = 4 , help="Create a thumbnail. ",
                       type=str)
    group.add_argument("-d", "--dir", nargs = 4 , help="Create a thumbnail of all the images in a directory. ",
                       type=str)
